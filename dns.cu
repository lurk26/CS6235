...
vector<vector<float> > u(nx, vector<float>(ny + 1));
    ...
    for (int i = 0; i < nx; i++)
    {
        for (int j = 0; j < ny; j++)
        {
            vc_cpu[i][j] = 1.0 / 2.0*(v[i + 1][j] + v[i][j]);
            pc_cpu[i][j] = 1.0 / 4.0*(p[i][j] + p[i + 1][j] + p[i][j + 1] + p[i + 1][j + 1]);
            uc_cpu[i][j] = 1.0 / 2.0*(u[i][j] + u[i][j + 1]);
            omc_cpu[i][j] = 1.0 / dx*(v[i + 1][j] - v[i][j]) - 1.0 / dy*(u[i][j + 1] - u[i][j]);
            Tc_cpu[i][j] = 1.0 / 4.0*(T[i][j] + T[i + 1][j] + T[i][j + 1] + T[i + 1][j + 1]);
    ...
    for (int j = 0; j < ny; j++)
    {
        for (int i = 0; i < nx; i++)
        {
            f << setw(15) << t - dt << setw(15) << i*dx << setw(15) << j*dy << setw(15) << uc_cpu[i][j] << setw(15) << vc_cpu[i][j] << setw(15) << pc_cpu[i][j] << setw(15) << Tc_cpu[i][j] * T_amb - 273.15 << setw(15) << omc_cpu[i][j] << endl;
        } // end for i
    } // end for j
    //.........................................................................................................
